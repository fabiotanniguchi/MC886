...
def load_training_testing_data(X, Y):
    ...
    if (True == USE_TEST_FILE):
        ...
        x_test, y_test = load_model_data(TESTING_MODEL_FILE)
...
def predict_linear_model(x_train, y_train, x_test, y_test):
    ...
    #x.plot([1920, 2020], [1920, 2020], 'k--', lw = 4)
    ...
    train_sizes, train_scores, test_scores = learning_curve(
      lm, x_train, y_train, train_sizes=[0.2, 0.5, 0.7], cv=4)
...
def predict_sgd_model(x_train, y_train, x_test, y_test):
    ...
    for index, ite in enumerate(iterations):
        sgd = linear_model.SGDRegressor(alpha=0.0001, average=False, epsilon=0.1, eta0=0.01,
